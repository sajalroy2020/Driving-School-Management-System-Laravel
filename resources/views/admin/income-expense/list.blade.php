...
@section('content')
    <div data-aos="fade-up" data-aos-duration="1000" class="p-sm-30 p-15">
        <div class="row rg-20">
            <div class="col-12">
                <div class="bg-body-tertiary border-bottom rounded-4">
                    <div class="row rg-20">
                        <div class="col-xl-4 col-sm-6">
                            <div class="">
                                <div class="dashboard-card-content">
                                    <h4 class="title">{{showCurrency($getIncomeExpenseInfo['total_income'])}}</h4>
                                    <p class="info">{{__("Total Income")}}</p>
                                </div>
                            </div>
                        </div>
                        <div class="col-xl-4 col-sm-6">
                            <div class="">
                                <div class="dashboard-card-content">
                                    <h4 class="title">{{showCurrency($getIncomeExpenseInfo['total_expense'])}}</h4>
                                    <p class="info">{{__("Total Expense")}}</p>
                                </div>
                            </div>
                        </div>
                        <div class="col-xl-4 col-sm-6">
                            <div class="">
                                <div class="dashboard-card-content">
                                    <h4 class="title">{{showCurrency($getIncomeExpenseInfo['account_balance'])}}</h4>
                                    <p class="info">{{__("Account Balance")}}</p>
                                </div>
                            </div>
                        </div>
                    </div>
            ...
            <div class="col-12">
                <div class="bd-ra-15 bg-white p-sm-30 p-15 mb-30">
                    <div class="table-wrapTop d-flex align-items-center justify-content-center justify-content-md-between flex-wrap g-10 pb-18">
                        ...
                        <button class="py-13 pr-15 pl-10 bd-one bd-c-primary-color bg-primary-color bd-ra-4 fs-14 fw-500 lh-14 text-white" data-bs-toggle="modal" data-bs-target="#addIncomeExpenseModal">+ {{__('Add Income/Expense')}}</button>
                    </div>
                    <table class="table zTable zTable-last-item-right" id="incomeExpenseDataTable">
                        <thead>
                            <tr>
                                <th>
                                    <div class="text-nowrap">{{__('Income/Expense Id')}}</div>
                                </th>
                                <th>
                ...
                <div class="row rg-20 pb-25">
                    <div class="col-md-12">
                        <div class="zForm-wrap">
                            <label for="asPricing" class="sf-form-label">{{__('Type')}} <span
                                    class="text-danger">*</span></label>
                            <select class="sf-select-without-search" name="type">
                                <option>{{__("Select Type")}}</option>
                                <option value="{{INCOME_EXPENSE_TYPE_INCOME}}">{{__('Income')}}</option>
                                <option value="{{INCOME_EXPENSE_TYPE_EXPENSE}}">{{__('Expense')}}</option>
                            </select>
                        </div>
                    </div>
                    <div class="col-12">
    ...
    <input type="hidden" id="income-expense-route" value="{{route('admin.income-expense.list')}}">
@endsection
@push('script')
