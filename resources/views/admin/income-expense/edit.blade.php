...
<form class="ajax-request reset" action="{{route('admin.income-expense.store')}}" method="POST"
    ...
    <div class="row rg-20 pb-25">
        <div class="col-md-12">
            <div class="zForm-wrap">
                ...
                <select class="sf-select-without-search" name="type">
                    ...
                    <option value="{{INCOME_EXPENSE_TYPE_INCOME}}" {{$income_expense_data->type == INCOME_EXPENSE_TYPE_INCOME?'selected':''}}>{{__('Income')}}</option>
                    <option value="{{INCOME_EXPENSE_TYPE_EXPENSE}}" {{$income_expense_data->type == INCOME_EXPENSE_TYPE_EXPENSE?'selected':''}}>{{__('Expense')}}</option>
                </select>
            </div>
        </div>
        <div class="col-12">
        ...
        @if ($income_expense_data->status == STATUS_PENDING)
            <div class="col-12">
                <label for="addRoleStatus" class="sf-form-label">{{__('Status')}}</label>
                <select class="sf-select-without-search" name="status">
                    <option value="{{STATUS_APPROVED}}">{{__('Approved')}}</option>
                    <option value="{{STATUS_CANCEL}}">{{__('Canceled')}}</option>
                </select>
            </div>
        @endif
