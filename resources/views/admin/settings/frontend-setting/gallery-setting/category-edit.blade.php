  ...
  <input type="hidden" name="id" value="{{encrypt($category->id)}}">
    ...
    <div class="row rg-20 pb-25">
      <div class="col-12">
        ...
        <input type="text" value="{{$category->category_name}}" name="category_name" class="form-control sf-form-control" placeholder="{{__('Enter Category Name')}}" />
      </div>
      <div class="col-12">
        ...
        <select class="sf-select-without-search" name="status">
            ...
            <option {{$category->status == STATUS_DEACTIVATE ? 'selected' : ''}} value="{{STATUS_DEACTIVATE}}">{{__('Deactive')}}</option>
        </select>
      </div>
    </div>
