...
class GeneralPaymentService
{
    ...
    {
        if ($status == 'student'){
            $listData = Payment::where(['student_id'=> auth()->id()])->with(['student']);
        }else{
            $listData = Payment::where(['tenant_id'=> auth()->user()->tenant_id])->with(['student']);
        }
        ...
        return datatables($listData)
            ...
            ->addColumn('payable_amount', function ($data) {
                    $amount = getPayableAmountByPaymentId($data->id, $data->enrolment_id, $data->student_id);
                    return showCurrency($amount);
            })
            ->editColumn('payment_type', function ($data) {
                if ($data->payment_type == PAYMENT_TYPE_OFFLINE){
                    return "<td>".__("Cash"). "</td>";
                }else{
                    return "<td>".__("Online"). "</td>";
                }
            })
            ...
            ->addColumn('due_amount', function ($data) {
                $amount = getDueAmountByPaymentId($data->id, $data->enrolment_id, $data->student_id);
                return showCurrency($amount);
            })
            ...
            ->addColumn('action', function ($data) use($status) {
                ...
                if ($status == 'student'){
                    $payNow='';
                    if ($data->payment_type == PAYMENT_TYPE_ONLINE){
                        $payNow = '<li>
                                        <a href="' . route('admin.payment.view', encrypt($data->id)) . '" class="border-0 bg-transparent p-0 d-flex align-items-center cg-8">
                                            <img src="' . asset('assets/images/icon/eye.svg') . '" alt="edit" />
                                            <p class="fs-14 fw-500 lh-19 text-main-color">' . __("Pay Now") . '</p>
                                        </a>
                                    </li>';
                    }
                    $html = '<div class="dropdown dropdown-one">
                                ...
                                <ul class="dropdown-menu dropdownItem-one">
                                    <li>
                                        <button onclick="editCommonModal(\'' . route('student.payment.view', encrypt($data->id)) . '\'' . ', \'#details-modal\')" type="button" class="border-0 bg-transparent p-0 d-flex align-items-center cg-8">
                                            <img src="' . asset('assets/images/icon/eye.svg') . '" alt="edit" />
                    ...
                    $html = '<div class="dropdown dropdown-one">
                                ...
                                <ul class="dropdown-menu dropdownItem-one">
                                    <li>
                                        <button onclick="editCommonModal(\'' . route('admin.payment.edit', encrypt($data->id)) . '\'' . ', \'#edit-modal\')" class="border-0 bg-transparent p-0 d-flex align-items-center cg-8">
                                            <div class="d-flex">
                                    ...
                                    <li>
                                        <button onclick="deleteCommonMethod(\'' . route('admin.payment.delete', encrypt($data->id)) . '\', \'paymentDataTable\')" class="border-0 bg-transparent p-0 d-flex align-items-center cg-8">
                                            <div class="d-flex">
            ...
            ->rawColumns(['student_name', 'action', 'status','payment_date', 'payable_amount', 'payment_type', 'payment_amount', 'due_amount'])
            ->make(true);
    }
    ...
    {
        return Payment::with(['package', 'student'])->find($id);
    }
    ...
    public function store($request)
    {
        ...
        try {
            $id = $request->get('id', 0);
            $getEnrolmentData = Enrolment::where(['id'=>$request->enrolled_student, 'tenant_id'=>auth()->user()->tenant_id])->first();
            if (!is_null($getEnrolmentData)){
                if ($id != 0){
                    $dataObj = Payment::find(decrypt($id));
                ...
                $dataObj->enrolment_id = $getEnrolmentData->id;
                $dataObj->student_id = $getEnrolmentData->student_id;
                $dataObj->package_id = $getEnrolmentData->package_id;
                $dataObj->amount = $request->amount;
                $dataObj->contact_amount = $getEnrolmentData->payable_amount;
                $dataObj->payment_type = $request->payment_type;
                $dataObj->description = $request->note;
                $dataObj->tenant_id = auth()->user()->tenant_id;

                if ($request->payment_type == PAYMENT_TYPE_OFFLINE) {
                    $dataObj->status = PAYMENT_STATUS_SUCCESS;
                }else{
                    $dataObj->status = isset($request->status) ? $request->status : PAYMENT_STATUS_PENDING;
                }
                $dataObj->save();

                if ($id == 0) {
                    $paymentId = 'PID' . sprintf('%06d', $dataObj->id);
                    Payment::where('id', $dataObj->id)->update(['payment_id' => $paymentId]);
                }
            }
            DB::commit();
    ...
    public function getDueAmountByStudentOrEnrolmentId($id, $condition=null)
    {
        try {
            if ($condition == 'student'){
                $data = Payment::where('student_id', $id)->get();
            }else{
                $data = Payment::where('enrolment_id', $id)->get();
            }
            $paidTotal = 0;
            if (!empty($data)){
            ...
            $data['total_paid'] = $paidTotal;
            return $this->successResponse($data, __(MSG_DELETED_SUCCESSFULLY));
        }catch (Exception $exception){
            return $this->errorResponse([], __(MSG_SOMETHING_WENT_WRONG));
        }
    }
