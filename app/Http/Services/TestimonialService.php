...
class TestimonialService
{
    ...
    {
        $getData = Testimonial::where('tenant_id', auth()->user()->tenant_id);
        ...
        return datatables($getData)
            ...
            ->addColumn('action', function ($data) {
                return '<div class="dropdown dropdown-one">
                            ...
                            <ul class="dropdown-menu dropdownItem-one">
                                <li>
                                    <button onclick="editCommonModal(\'' . route('admin.setting.frontend-setting.testimonial.edit', encrypt($data->id)) . '\'' . ', \'#editModal\')" class="border-0 bg-transparent p-0 d-flex align-items-center cg-8" data-bs-toggle="modal">
                                        <div class="d-flex">
                                ...
                                <li>
                                    <button onclick="deleteCommonMethod(\'' . route('admin.setting.frontend-setting.testimonial.delete', encrypt($data->id)) . '\', \'testimonialDataTable\')" class="border-0 bg-transparent p-0 d-flex align-items-center cg-8">
                                        <div class="d-flex">
            ...
            ->rawColumns(['image', 'name', 'title', 'action', 'status'])
            ->make(true);
    }
    ...
    public function store($request)
    {
        ...
        try {
            ...
            if ($request->student_id != null) {
                $student = Enrolment::where('student_id', $request->student_id)->with('student')->first();
            ...
            $testimonial->student_type = $request->student_type;
            $testimonial->quotation_text = $request->quotation_text;
            $testimonial->title = $request->title;
            $testimonial->comment = $request->comment;

            if ($testimonial->student_type == 1) {
                $testimonial->student_id = $request->student_id;
                $testimonial->image = $student->student->picture;
                $testimonial->name = $student->student->name;
                $testimonial->package_name = $package->package_name;
            }else{
                $testimonial->name = $request->name;
                $testimonial->package_name = null;
                $testimonial->student_id = null;
            ...
            $testimonial->tenant_id = auth()->user()->tenant_id;
            $testimonial->status = isset($request->status) ? $request->status : STATUS_ACTIVE;
            $testimonial->save();
            DB::commit();
