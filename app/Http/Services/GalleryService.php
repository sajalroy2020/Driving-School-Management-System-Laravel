...
class GalleryService
{
    ...
    {
        $getData = Gallery::with('galleryCategory')->where('tenant_id', auth()->user()->tenant_id);
        ...
        return datatables($getData)
            ...
            ->addColumn('category_name', function ($getData) {
                return $getData->galleryCategory->category_name;
            })
            ...
            ->addColumn('action', function ($data) {
                return '<div class="dropdown dropdown-one">
                            ...
                            <ul class="dropdown-menu dropdownItem-one">
                                <li>
                                    <button onclick="editCommonModal(\'' . route('admin.setting.frontend-setting.gallery.edit', encrypt($data->id)) . '\'' . ', \'#editModal\')" class="border-0 bg-transparent p-0 d-flex align-items-center cg-8" data-bs-toggle="modal">
                                        <div class="d-flex">
                                ...
                                <li>
                                    <button onclick="deleteCommonMethod(\'' . route('admin.setting.frontend-setting.gallery.delete', encrypt($data->id)) . '\', \'galleryDataTable\')" class="border-0 bg-transparent p-0 d-flex align-items-center cg-8">
                                        <div class="d-flex">
            ...
            ->rawColumns(['image', 'category_name', 'title', 'action', 'status'])
            ->make(true);
    }
    ...
    public function store($request)
    {
        ...
        try {
            ...
            if ($request->hasFile('image')) {
                ...
                $uploaded = $new_file->upload('gallery', $request->image);
                $gallery->image = $uploaded->id;
            }

            $gallery->title = $request->title;
            $gallery->category_id = $request->category_id;
            $gallery->tenant_id = auth()->user()->tenant_id;
            $gallery->status = isset($request->status) ? $request->status : STATUS_ACTIVE;
            $gallery->save();
            DB::commit();
