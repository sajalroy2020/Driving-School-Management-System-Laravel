...
class StudentPackageService
{
    ...
    {
        $getData = StudentPackage::with(['student', 'packages', 'paymentGateway'])->get();
        ...
        return datatables($getData)
            ...
            ->addColumn('email', function ($getData) {
                return $getData->student->email;
            })
            ->addColumn('package_name', function ($getData) {
                return $getData->packages->category->category_name;
            })
            ->addColumn('payment_gateway', function ($getData) {
                return $getData->paymentGateway->title;
            })
            ...
            ->addColumn('action', function ($data) {
                return '<div class="dropdown dropdown-one">
                                ...
                                <ul class="dropdown-menu dropdownItem-one">
                                    <li>
                                        <button onclick="editCommonModal(\'' . route('admin.student-packages.edit', encrypt($data->id)) . '\'' . ', \'#edit-modal\')" class="border-0 bg-transparent p-0 d-flex align-items-center cg-8" data-bs-toggle="modal" data-bs-target="#edit-modal">
                                            <div class="d-flex">
                                    ...
                                    <li>
                                        <button onclick="deleteCommonMethod(\'' . route('admin.student-packages.delete', encrypt($data->id)) . '\', \'stdPackagesDataTable\')" class="border-0 bg-transparent p-0 d-flex align-items-center cg-8">
                                            <div class="d-flex">
            ...
            ->rawColumns(['name', 'email', 'package_name', 'payment_gateway', 'action', 'status'])
            ->make(true);
    }

    public function getById($id)
    {
        return StudentPackage::find($id);
    }
    ...
    public function store($request)
    {
        ...
        try {
            ...
            if ($id != 0) {
                $id = decrypt($id);
                $std_packages = StudentPackage::find($id);
                $msg = __(MSG_UPDATED_SUCCESSFULLY);
            } else {
                $std_packages = new StudentPackage();
                $msg = __(MSG_CREATED_SUCCESSFULLY);
            }

            $std_packages->student_id = $request->student_id;
            $std_packages->package_id = $request->package_id;
            $std_packages->payment_id = $request->payment_id;
            $std_packages->status = isset($request->status) ? $request->status : USER_STATUS_ACTIVE;
            $std_packages->tenant_id = $authUser->tenant_id;
            $std_packages->save();
    ...
    public function destroy($id)
    {
        try {
            DB::beginTransaction();
            $std_package = StudentPackage::find($id);
            $std_package->delete();
            DB::commit();
    ...
    public function myCurrentPackage(){
        return Enrolment::where(['student_id'=> auth()->id(), 'status'=> ENROLMENT_RUNNING])->first();
    }
