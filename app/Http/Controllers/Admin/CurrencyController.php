...
class CurrencyController extends Controller
{
    ...
    public function index(Request $request)
    {
        if ($request->ajax()) {
            ...
            return datatables($currencyList)
                ->addIndexColumn()
                ->editColumn('currency_code', function ($data) {
                    $currencyCode = $data->currency_code;
                    if ($data->default_currency == STATUS_ACTIVE) {
                        $currencyCode = $currencyCode . ' <b>(' . __("Default Currency") . ')';
                    }
                    return $currencyCode;
                })
                ->editColumn('currency_placement', function ($data) {

                    if ($data->currency_placement == CURRENCY_PLACEMENT_AFTER) {
                        return "After";
                    } else {
                        return "Before";
                    }
    ...
    public function store(CurrencyRequest $request)
    {
        ...
        try {
            ...
            if ($request->default_currency) {
                $is_default = 1;
                Currency::where('default_currency', STATUS_ACTIVE)->update(['default_currency' => STATUS_PENDING]);
            }
            ...
            $currency->default_currency = $is_default;
            $currency->save();
    ...
    public function edit($id)
    {
        try {
            ...
            return view('admin.settings.currency.edit_form', $data)->render();
        } catch (Exception $exception) {
            return $this->errorResponse([], __(MSG_SOMETHING_WENT_WRONG));
        }
    ...
    public function delete($id)
    {
        try {
            ...
            if ($currency->default_currency == 1) {
                return $this->errorResponse([],__('You Cannot delete default currency'));
            }
            $currency->delete();
