(function ($) {
    ...
    var studentListTable;
        $(document).on("input", "#studentListTableSearch", function () {
            studentListTable.search($(this).val()).draw();
        });

    studentListTable = $("#studentDataTable").DataTable({
        pageLength: 10,
        ordering: false,
        serverSide: true,
        processing: true,
    ...
    function studentEditResponse(response){
        commonResponse(response);
        window.location.reload();
    }

    window.studentEditResponse = studentEditResponse;
