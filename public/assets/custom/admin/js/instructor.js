(function ($) {
    ...
    var instructorListTable;
        $(document).on("input", "#instructorListTableSearch", function () {
            instructorListTable.search($(this).val()).draw();
        });

    instructorListTable = $("#instructorDataTable").DataTable({
        pageLength: 10,
        ordering: false,
        serverSide: true,
        processing: true,
        ...
        ajax: $('#instructor-list-route').val(),
        language: {
            paginate: {
    ...
    function instructorEditResponse(response){
        commonResponse(response);
        window.location.reload();
    }
    window.instructorEditResponse = instructorEditResponse;
